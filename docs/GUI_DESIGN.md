...
左侧导航保持稳定，顶部显示制品版本、扫描状态、依赖模式和当前租户/身份；任何页面都能回到“当前扫描上下文”。
...
当前完成：React/TypeScript/Vite 原型、项目总览、入口地图、路径/发现/攻击链视图、真实 API DTO 校验和 SSE 订阅边界。当前未完成：生产级鉴权、多租户、持久化、动态执行、沙箱、LLM 和真实漏洞验证。
