...
根增量（2026-07-29，`PARTIAL`，声明范围=GuardSurface 目录驱动 FORCED allowlist + AccessControl 短接，Docker-only）：静态 `GuardSurfaceCatalog` 从制品 JAR（含有界 BOOT-INF/lib shiro/spring-security）收割 `GUARD:AUTH:<simpleName>` + typeNames；`ProbePlanService` 将 catalog refs 作 `guardHints` 写入 FORCED `forcedGuardRefs`（不再恒为空）；executor 注入有界 `-Dveyrion.sandbox.forcedGuardTypeNames`；agent allowlist 非空时只强达匹配类型，空则回退启发式；P1 `AccessControlAdvice` 对 `isAccessAllowed` 在 FORCED 下返回 true（Shiro AccessControl / LoginFilter）。验收：`GuardSurfaceCatalogAcceptanceTest` + 更新后的 `ForcedReachabilityGuardAcceptanceTest`。限制：Interceptor `preHandle` 改写、登录状态机、rememberMe AES mint、OSS 全链路仍开放；不得升 VERIFIED。
