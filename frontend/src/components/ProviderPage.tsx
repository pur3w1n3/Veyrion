...
export function ProviderPage({ projectId }: { projectId: string }) {
  ...
  return <section>
    <PageHeader eyebrow="PROVIDERS / MODELS" title="模型服务">
      管理 Provider、模型清单与项目 AI 角色绑定；凭据只由后端加密保存。
    </PageHeader>
    ...
    <article className="panel">
      ...
      <form className="stack-form" onSubmit={saveProvider} autoComplete="off">
        ...
        <p className="form-help">提交后密钥输入立即清空。非本机 HTTP 会明文传输 API Key 和模型数据，仅应连接受信内网网关。</p>
