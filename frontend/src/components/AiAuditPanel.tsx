...
export function AiAuditPanel({ projectId }: { projectId: string }) {
  ...
  const recreateJobs = () => {
    if (!projectId || !window.confirm('为当前项目和最新扫描重新创建四个 AI 角色任务？旧失败任务将作为历史记录保留。')) return
    setCreating(true)
    setError(undefined)
    void Promise.allSettled(roles.map((role) => api.createAiJob(projectId, { role, authorized: true })))
      .then((results) => {
  ...
  return <article className="panel section-gap">
    <div className="panel-head">
      ...
      <div className="button-row">
        <button className="secondary-button" type="button" disabled={creating || !projectId} onClick={recreateJobs}>{creating ? '创建中…' : '重新创建四角色任务'}</button>
