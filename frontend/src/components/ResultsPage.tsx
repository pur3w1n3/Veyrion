...
import {
  CoverageMatrixPanel,
  EvidenceGraphPanel,
  HypothesisFamilyPanel
} from './CapabilityEvidencePanels'
...
export function ResultsPage({ projectId, snapshot, language }: { projectId: string; snapshot: DashboardSnapshot | null; language: OutputLanguage }) {
  ...
  const familyOfFinding = (finding: { hypothesisId?: string; securityProperty?: string; sinkId?: string; sink?: string; title?: string }): HypothesisFamily => {
  ...
  const filteredFindings = useMemo(() => {
    ...
    return findings.filter((finding) => {
      ...
      if (findingFamily !== 'ALL' && familyOfFinding(finding) !== findingFamily) return false
  ...
  const selectedFindingFamily = selectedFinding ? familyOfFinding(selectedFinding) : undefined
  ...
  const downloadHtml = () => {
    ...
    const rows = findings.map((finding) => `<tr><td>${esc(finding.severity)}</td><td>${esc(finding.title)}</td><td>${esc(familyOfFinding(finding))}</td><td>${esc(finding.entry)}</td><td>${esc(finding.sink)}</td><td>${esc(finding.status)}</td></tr>`).join('')
  ...
  const viewMeta = (view: ResultsView): { label: string; eyebrow: string; count: number; blurb: string } => {
    ...
    switch (view) {
      case 'report':
        return { label, eyebrow: english ? 'PRIMARY' : '主视图', count: reportSummary ? 1 : 0, blurb }
      case 'pathRuns':
        return { label, eyebrow: english ? 'SESSIONS' : '路径会话', count: pathRuns.length, blurb }
      case 'coverage':
        return { label, eyebrow: 'COVERAGE', count: coverage?.gaps?.total ?? coverage?.gaps?.unreached ?? 0, blurb }
      case 'hypotheses':
        return { label, eyebrow: 'HYPOTHESIS', count: hypotheses.length, blurb }
      case 'evidenceGraph':
        return { label, eyebrow: 'GRAPH', count: evidenceGraph?.nodes.length ?? evidenceGraph?.nodeCount ?? 0, blurb }
      case 'contrast':
        return { label, eyebrow: english ? 'TRIAGE' : '研判对照', count: rankedSinks.length, blurb }
      case 'findings':
        return { label, eyebrow: english ? 'EVIDENCE' : '证据详情', count: secondaryFindingCount, blurb }
      case 'verified':
        return { label, eyebrow: english ? 'GATED' : '门禁', count: verifiedFindings.length, blurb }
      case 'experiments':
        return { label, eyebrow: english ? 'D3 / PLANS' : '实验卡', count: sqlCards.length + experimentPlans.length, blurb }
    }
  }
      ...
      <article className="panel">
        <div className="panel-head">
          ...
          <div className="button-row">
            ...
            {reportSummary && <>
              <button type="button" className="secondary-button" onClick={downloadReport} title={DOWNLOAD_ARTIFACTS.reportMarkdown.id}>
                {english ? DOWNLOAD_ARTIFACTS.reportMarkdown.en : DOWNLOAD_ARTIFACTS.reportMarkdown.zh}
              </button>
              <button type="button" className="secondary-button" onClick={downloadHtml} title={DOWNLOAD_ARTIFACTS.findingsHtml.id}>
                {english ? DOWNLOAD_ARTIFACTS.findingsHtml.en : DOWNLOAD_ARTIFACTS.findingsHtml.zh}
              </button>
            </>}
            {snapshot && <button type="button" className="secondary-button" onClick={downloadJson} title={DOWNLOAD_ARTIFACTS.dashboardJson.id}>
              {english ? DOWNLOAD_ARTIFACTS.dashboardJson.en : DOWNLOAD_ARTIFACTS.dashboardJson.zh}
            </button>}
          </div>
        </div>
        ...
        <div className="section-gap">
          <p className="eyebrow">{english ? 'HYPOTHESIS FAMILIES' : '假设族'}</p>
          <p className="form-help">
            {english
              ? `API hypotheses[] · ${hypotheses.length} rows. Empty families stay listed; unknown wire values degrade to UNKNOWN.`
              : `API hypotheses[] · ${hypotheses.length} 条。空族仍列出；未知 wire 值降级为 UNKNOWN。`}
          </p>
          <div className="card-list">
            {HYPOTHESIS_FAMILIES.map((family) => {
              const items = hypothesesByFamily.get(family) ?? []
              return <div className="list-card" key={family}>
                <div>
                  <strong>{family}</strong>
                  <small>
                    {items.length === 0
                      ? (english ? 'No hypotheses in this family yet (not a claim of safety).' : '该族暂无假设（不等于安全）。')
                      : items.map((item) => item.hypothesisId).join(', ')}
                  </small>
                </div>
    ...
    {activeView === 'findings' && <>
      <div className="result-grid">
        <article className="panel">
          ...
          <div className="finding-toolbar"><label className="field"><span>{english ? 'Filter findings' : '筛选发现'}</span><input value={findingQuery} onChange={(event) => setFindingQuery(event.target.value)} placeholder={english ? 'Title, entry, sink, family...' : '标题、入口、sink 或 family…'} /></label><label className="field"><span>{english ? 'Evidence status' : '证据状态'}</span><select value={findingStatus} onChange={(event) => setFindingStatus(event.target.value as typeof findingStatus)}><option value="ALL">{english ? 'All statuses' : '全部状态'}</option><option value="STATIC_INFERRED">STATIC_INFERRED</option><option value="DYNAMIC_SUSPECTED">DYNAMIC_SUSPECTED</option><option value="DYNAMIC_CONFIRMED">DYNAMIC_CONFIRMED</option><option value="VERIFIED">VERIFIED</option><option value="UNREACHED">UNREACHED</option></select></label><label className="field"><span>{english ? 'Hypothesis family' : '假设族'}</span><select value={findingFamily} onChange={(event) => setFindingFamily(event.target.value as typeof findingFamily)}><option value="ALL">{english ? 'All families' : '全部族'}</option>{HYPOTHESIS_FAMILIES.map((family) => <option key={family} value={family}>{family}</option>)}</select></label><label className="field checkbox-field"><span>{english ? 'Show AUTH_GAP' : '显示 AUTH_GAP'}</span><input type="checkbox" checked={showAuthGap} onChange={(event) => setShowAuthGap(event.target.checked)} /></label></div>
          <div className="card-list">{filteredFindings.map((finding) => <button type="button" className={`finding-card finding-card-button status-tone-${finding.status.toLowerCase()} ${finding.id === selectedFinding?.id ? 'selected' : ''}`} key={finding.id} onClick={() => { setSelectedFindingId(finding.id); setSelectedStepIndex(0) }}><div className={`severity severity-${finding.severity}`}>{finding.severity}</div><div><strong>{finding.title}</strong><small>{familyOfFinding(finding)} · {finding.entry} → {finding.sink}</small><small>{finding.evidence} {english ? 'evidence items' : '条证据'} · {finding.dependency === 'none' && !english ? '无外部依赖记录' : finding.dependency}</small></div><StatusPill status={finding.status} /></button>)}{filteredFindings.length === 0 && <p className="empty-state">{english ? 'No findings match the current filter.' : '没有符合当前筛选条件的发现。'}</p>}</div>
        </article>
        <article className="panel">
