...
import { api, type DashboardSnapshot, type Entry, type EvidenceDto, type Finding, type PathStep, type PathTrace, type VerificationStatus } from './api'
...
function Paths({ path, paths, entries, dependencyMode }: { path: PathStep[]; paths: PathTrace[]; entries: Entry[]; dependencyMode?: string }) {
  const [selectedPathId, setSelectedPathId] = useState<string | null>(null)
  const selectedPath = paths.find((candidate) => candidate.pathId === selectedPathId) ?? paths[0]
  const activePath = selectedPath?.steps ?? path
  const current = entries.find((entry) => entry.id === selectedPath?.entrypointId) ?? entries[0]
  const activeDependencyMode = selectedPath?.dependencyMode ?? dependencyMode
  const pathStatus = selectedPath?.verificationStatus ?? current?.status ?? 'UNREACHED'
  const hasDynamicMetadata = selectedPath && (
    selectedPath.fixtureOnly !== undefined ||
    selectedPath.requiredCapability !== undefined ||
    selectedPath.taskId !== undefined ||
    selectedPath.dynamicExecutionMode !== undefined
  )
  return <section className="page-section"><div className="page-title"><div><p className="eyebrow">TRACE EVIDENCE / {selectedPath?.pathId ?? current?.id ?? 'UNSCANNED'}</p><h1>路径探索器</h1><p>入口参数、分支约束和外部依赖的可追溯时间线；运行状态仅来自 dashboard GET。</p></div><div className="title-actions"><span className="mode-badge">{selectedPath?.dynamicExecutionMode ?? 'STATIC ONLY'}</span><button className="primary-button" disabled title="动态任务由 Control Plane 管理">↻ 动态重放（待接入）</button></div></div>{selectedPath?.fixtureOnly === true && <div className="fixture-warning"><strong>FIXTURE ONLY</strong><span>受控仓库 Fixture，非用户导入制品</span></div>}<div className="trace-layout"><aside className="path-tree panel"><div className="panel-head"><h2>路径树</h2><span>{paths.length || entries.length} 条路径</span></div>{paths.length > 0 ? paths.map((candidate) => <button type="button" className={`tree-node tree-button ${candidate.pathId === selectedPath?.pathId ? 'selected' : ''} ${candidate.verificationStatus === 'UNREACHED' ? 'muted-tree' : ''}`} key={candidate.pathId} onClick={() => setSelectedPathId(candidate.pathId)}><i />{candidate.pathId}<small>{candidate.entrypointId} · {candidate.stopReason}</small></button>) : entries.map((entry, index) => <div className={`tree-node ${index === 0 ? 'selected' : ''} ${entry.status === 'UNREACHED' ? 'muted-tree' : ''}`} key={entry.id}><i />{entry.method} {entry.route}<small>{entry.precondition} · {entry.coverage}%</small></div>)}{paths.length === 0 && entries.length === 0 && <div className="tree-node muted-tree"><i />暂无入口<small>等待 Control Plane 扫描</small></div>}</aside><main className="trace-main panel"><div className="trace-head"><div><p className="eyebrow">TRACE TIMELINE</p><h2>{activePath[0]?.label ?? '暂无路径证据'}{activePath.length > 1 ? ` → ${activePath[activePath.length - 1].label}` : ''}</h2></div><StatusPill status={pathStatus} /></div>{selectedPath && <div className="path-stop"><span>STOP REASON</span><strong>{selectedPath.stopReason}</strong>{selectedPath.preconditions.length > 0 && <small>前置条件：{selectedPath.preconditions.join(' · ')}</small>}</div>}{hasDynamicMetadata && <div className="dynamic-meta"><span>task <b>{selectedPath.taskId ?? '未提供'}</b></span><span>capability <b>{selectedPath.requiredCapability ?? '未提供'}</b></span><span>mode <b>{selectedPath.dynamicExecutionMode ?? '未提供'}</b></span></div>}<div className="trace-line">{activePath.map((step, index) => <div className={`trace-step ${step.state}`} key={`${step.sequence ?? index}-${step.label}`}><div className={`trace-dot dot-${step.kind}`}><span>{step.sequence ?? index + 1}</span></div><div className="trace-card"><div><strong>{step.label}</strong><span className={`kind-label kind-${step.kind}`}>{step.kind}</span></div><p>{step.detail}</p>{(step.provenanceKind || step.eventType || step.verificationStatus || step.kind === 'dependency') && <div className="trace-meta"><span>evidence refs <b>{step.evidenceRefs?.length ?? 0}</b></span>{step.provenanceKind && <span>provenance <b>{step.provenanceKind}</b></span>}{step.eventType && <span>event <b>{step.eventType}</b></span>}<span>mode <b>{activeDependencyMode ?? 'UNKNOWN'}</b></span>{step.verificationStatus && <StatusPill status={step.verificationStatus} />}</div>}</div></div>)}{activePath.length === 0 && <p className="trace-empty">暂无可展示路径；请先创建扫描并等待证据。</p>}</div></main></div></section>
}
