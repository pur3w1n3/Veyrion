-- Expand fixed agent roles with AUTH_ANALYSIS (path-debug auth + synthetic identity).
...
-- Preserve prompt_zh / prompt_en columns introduced in V010.

CREATE TABLE project_ai_role_bindings_v012 (
    project_id TEXT NOT NULL,
    role TEXT NOT NULL CHECK (role IN (
        'PRE_ANALYSIS','AUTH_ANALYSIS','PATH_EXPLORATION','DYNAMIC_VERIFICATION',
...
INSERT INTO project_ai_role_bindings_v012
SELECT project_id, role, workspace_id, provider_id, model, updated_at, prompt_zh, prompt_en
FROM project_ai_role_bindings;

DROP TABLE project_ai_role_bindings;
ALTER TABLE project_ai_role_bindings_v012 RENAME TO project_ai_role_bindings;

CREATE TABLE ai_jobs_v012 (
...
DROP TABLE ai_jobs;
ALTER TABLE ai_jobs_v012 RENAME TO ai_jobs;
