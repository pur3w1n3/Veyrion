...
- 主流程固定为：`制品摘要复核 → 静态事实/入口发现 → 前置建模 → 路径探索 → 断网动态观察 → 动态验证 → 漏洞研判 → 报告生成`。一次“开始审计”授权后由服务端流水线自动推进，无需逐步人工确认；任一步失败即停止后续自动推进。模型不能改写静态事实，也不能单独生成「已验证」。
- “模型服务”只负责接口密钥、模型清单和项目角色绑定。页面左侧显示并选择后端已保存的接口，右侧编辑连接与模型；保存角色绑定不再顺带创建任务。
- 模型对话过程嵌在「审计执行」页下方，不再作为左侧独立导航。页面以对话形式展示系统提示词、任务说明、模型思考（若返回）、工具调用与最终输出，底部显示实时动向。
- 编排器使用按角色固定的服务端任务说明：前置建模解释入口与业务；路径探索提出推测链路；动态验证必须基于路径探索结论独立对照动态证据；漏洞研判与报告生成继续受证据边界约束。模型不得发明路由或改写事实层。
- 组合入口仍以 `audit-runs` 启动；流水线在进程内武装，重启后未完成的自动接续不保证恢复。SQLite 保存 scan/job/事件事实。
...
- GUI：左侧移除独立「模型审计过程」；审计执行页内嵌对话区与底部实时动向；开始审计前需绑定全部五个角色。
